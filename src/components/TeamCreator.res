...
let make = (~update, ~renderWaitTime) => {
  let (teamInfo: Team.t, setTeamInfo) = React.useState((): Team.t => {name: "", captain: ""})
  ...
  let handleChange = (inputType: input, event) => {
    ...
    | Name =>
      setTeamInfo(currentTeamInfo => {
        ...currentTeamInfo,
        name: ReactEvent.Synthetic.target(event)["value"],
      })
    | Captain =>
      setTeamInfo(currentTeamInfo => {
        ...currentTeamInfo,
  ...
  <form className="register-team" onSubmit={handleSubmit(update)}>
    ...
    <label>
      ...
      <input type_="text" onChange={handleChange(Name)} value=teamInfo.name />
    </label>
    <label>
      {"Team Captain: "->React.string}
      <input type_="text" onChange={handleChange(Captain)} value=teamInfo.captain />
    </label>
    <button className="join-queue" type_="submit"> {"Join Queue"->React.string} </button>
  </form>
}
